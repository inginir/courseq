...
def generate(courses=[], locked_courses={}):
    all_courses_raw = courses if len(courses)>0 else get_courses_from_json()
    s0 = Sequence()
    s0.generate_terms(sepecific_limit = {"Summer2020": 0, "Summer2021":6, "Summer2022": 0})
    ...
    for _term in s0.terms:
        print(_term)
    s0.refill_all_terms(locked_courses)
    print("================= AFTER REFILL =======================")
    for _term in s0.terms:
        print(_term)
    s0.refill_all_terms(locked_courses)
    print("================= AFTER REFILL 2 =======================")
    for _term in s0.terms:
        print(_term)
    return s0
